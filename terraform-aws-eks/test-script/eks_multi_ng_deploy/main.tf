...
module eks_node_group {
    ...
    cluster_name              = var.cluster_name
    node_group_name           = "${var.node_group_name}-4_vCPU-16gb"
    ...
    labels                    = {node = "4vcpu", type = "spot"}
    cluster_create_wait       = module.eks_cluster.endpoint
    # ssh_key_name            = ""
    # tags                      = {}
}

module eks_node_group_2 {
    source                    = "../../modules/eks"
    cluster_name              = var.cluster_name
    node_group_name           = "${var.node_group_name}-8_vCPU-32gb"
    worker_node_role_arn      = module.eks_node_group.worker_node_role_arn
    private_subnet_id         = module.subnet[*].private_subnet
    desired_size              = 1
    max_size                  = 2
    min_size                  = 1
    ...
    labels                    = {node = "8vcpu", type = "spot"}
    cluster_create_wait       = module.eks_cluster.endpoint
    # ssh_key_name            = ""
    # tags                      = {}
    # worker_node_sg            = module.eks_node_group.node_sg_id
}
